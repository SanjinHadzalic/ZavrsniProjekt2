...
public class GenerateDocumentController {
    ...
    public ResponseEntity<?> uploadExcelFile(@RequestParam("file") MultipartFile file) {
        try {
            List<String> existingRingCodes = generateDocumentService.processExcelFile(file);
            if (existingRingCodes.isEmpty()) {
                return ResponseEntity.ok("File processed successfully");
            } else {
                return ResponseEntity.ok("File processed with existing ring codes: " + existingRingCodes);
            }
        } catch (IOException e) {
